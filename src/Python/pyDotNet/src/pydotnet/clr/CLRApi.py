...
class CLRApi:
    ...
    def __init__(self, hostname="localhost", port=56789, dll=None, server_args=[]):
        """
        Initialize CLR API
    ...
    def connect(self):
        ...
        def newServer():
            if "VIRTUAL_ENV" in os.environ.keys() and os.name == "nt":
            ...
            url = "svc://%s:%d" % (self.hostname, self.port)
            ...
            if SystemUtils.is_unix():
                ...
                self.pid = subprocess.Popen([server] + serverargs).pid
...
class TestCLRMessage(unittest.TestCase):
    ...
    def test_create(self):
        ...
        m1 = CLRMessage.createByObj(
            pd.DataFrame([[2.0, 0, 0], [0, 3.0, 0], [0, 0, 23.0]], index=['a', 'b', 'c'], columns=['a', 'b', 'c']))
